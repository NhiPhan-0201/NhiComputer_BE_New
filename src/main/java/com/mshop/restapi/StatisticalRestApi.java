...
public class StatisticalRestApi {
	...
	public ResponseEntity<List<Statistical>> getStatisticalAllDate() {
		...
		List<Statistical> listSta = new ArrayList<>();
		for (Object[] obj : list) {
			Double amount = obj[0] != null ? ((Number) obj[0]).doubleValue() : 0.0;
			Integer day = obj[1] != null ? ((Number) obj[1]).intValue() : 0;
			Date date = obj[2] instanceof Date ? (Date) obj[2] : null;
			BigInteger count = obj[3] != null ? new BigInteger(obj[3].toString()) : BigInteger.ZERO;
			Statistical sta = new Statistical(day, date, amount, count);
			listSta.add(sta);
		}
		return ResponseEntity.ok(listSta);
	}
	...
	public ResponseEntity<List<Statistical>> getStatisticalAllYear() {
		...
		for (Object[] obj : list) {
			Double amount = obj[0] != null ? ((Number) obj[0]).doubleValue() : 0.0;
			Integer year = obj[1] != null ? ((Number) obj[1]).intValue() : 0;
			Date date = obj[2] instanceof Date ? (Date) obj[2] : null;
			BigInteger count = obj[3] != null ? new BigInteger(obj[3].toString()) : BigInteger.ZERO;
			Statistical sta = new Statistical(year, date, amount, count);
			listSta.add(sta);
		}
		return ResponseEntity.ok(listSta);
	}
