...
public interface OrderRepository extends JpaRepository<Order, Long> {
	...
	@Query(value = "select sum(amount), year(order_date), min(order_date), count(id) from orders where status = 3 group by year(order_date)", nativeQuery = true)
	List<Object[]> getStatisticalYear();


	@Query(value = "select sum(amount), month(order_date), min(order_date), count(id) from orders where status = 3 group by month(order_date)", nativeQuery = true)
	List<Object[]> getStatisticalMonth();

	@Query(value = "select sum(amount), day(order_date), min(order_date), count(id) from orders where status = 3 group by day(order_date)", nativeQuery = true)
